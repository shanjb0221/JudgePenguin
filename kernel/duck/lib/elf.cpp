...
template <class T> static inline void push(uint64_t &rsp, const T &a) {
  rsp -= sizeof(a);
  *(T *)rsp = a;
}

static bool load_elf64(const char *buf, uint32_t len, const AppConfig &config,
                       App &app) {
  LDEBUG_ENTER_RET();
  ...
  for (uint64_t i = 0; i < e_phnum; i++) {
    ...
    if (p_filesz != 0) {
      if (p_offset > len || p_offset + p_filesz > len)
        return false;
    }

    // Check virtual address alignment [JudgeDuck-ABI, "Compilation and
    // Linkage"]
    if (p_align != PAGE_SIZE && p_align != HUGE_PAGE_SIZE)
      return false;

    // Check virtual address and size
    if (p_memsz < p_filesz)
      return false;
    if (p_vaddr + p_memsz < p_vaddr)
  ...
  if (min_vaddr < Memory::get_kernel_break())
    return false;
  if (max_vaddr <= min_vaddr || max_vaddr > Memory::get_vaddr_break())
    return false;

  const uint64_t memory_limit = round_up(config.memory_hard_limit, PAGE_SIZE);
  uint64_t memory_needed = max_vaddr - min_vaddr;
  if (memory_limit < memory_needed)
    return false;
  if (memory_limit < memory_needed + PAGE_SIZE)
    return false; // one page for STACK
  ...
  if (min_vaddr != load_vaddr_start) {
    LWARN(
        "Programs with load address other than 0x400000 are not supported yet");
    return false;
  }
  const uint64_t load_vaddr_break = load_vaddr_start + memory_limit;
  if (load_vaddr_break < load_vaddr_start)
    return false;
  if (load_vaddr_break > Memory::get_vaddr_break())
    return false;
  ...
  if (stdin_alloc_size < config.stdin_size)
    return false; // overflow?
  ...
  if (stdin_load_break < stdin_load_vaddr)
    return false; // overflow?
  special_region_break_curr = stdin_load_break;

  // Allocate stdout
  const uint64_t stdout_alloc_size =
      round_up(config.stdout_max_size, PAGE_SIZE);
  if (stdout_alloc_size < config.stdout_max_size)
    return false; // overflow?
  ...
  if (stdout_load_break < stdout_load_vaddr)
    return false; // overflow?
  special_region_break_curr = stdout_load_break;

  // Allocate stderr
  const uint64_t stderr_alloc_size =
      round_up(config.stderr_max_size, PAGE_SIZE);
  if (stderr_alloc_size < config.stderr_max_size)
    return false; // overflow?
  ...
  if (stderr_load_break < stderr_load_vaddr)
    return false; // overflow?
  ...
  const uint64_t ELF_image_load_break =
      ELF_image_load_vaddr + ELF_image_alloc_size;
  if (ELF_image_load_break < ELF_image_load_vaddr)
    return false;
  ...
  const uint64_t random_bytes_load_break =
      random_bytes_load_vaddr + random_bytes_alloc_size;
  if (random_bytes_load_break < random_bytes_load_vaddr)
    return false;
  ...
  if (special_region_break < special_region_vaddr)
    return false;
  if (special_region_break > Memory::get_vaddr_break())
    return false;
  ...
  Memory::set_page_flags_kernel(special_region_break,
                                Memory::get_vaddr_break());
  ...
  for (uint64_t i = 0; i < e_phnum; i++) {
    ...
    if (p_filesz != 0) {
      ...
      Memory::set_duck_written(round_down(p_vaddr, PAGE_SIZE),
                               round_up(p_vaddr + p_filesz, PAGE_SIZE));
    }
  }
  ...
  if (config.OB_need_clear) {
    ...
    Memory::set_duck_written(
        OB_load_vaddr, OB_load_vaddr + round_up(config.OB_size, PAGE_SIZE));
    memcpy((void *)OB_load_vaddr, config.OB_ptr, config.OB_size);
  }

  // [2020-11-18] Load ELF Image
  memcpy((void *)ELF_image_load_vaddr, hdr,
         std::min((uint32_t)len, (uint32_t)Memory::PAGE_SIZE));
  Memory::set_duck_written(ELF_image_load_vaddr, ELF_image_load_break);
  Memory::set_page_flags_user_readonly(ELF_image_load_vaddr,
                                       ELF_image_load_break);
  ...
  Memory::set_page_flags_user_readonly(random_bytes_load_vaddr,
                                       random_bytes_load_break);
...
bool load(const char *buf, uint32_t len, const AppConfig &config, App &app) {
...
RunResult run(const App &app, uint64_t time_limit_ns) {
  ...
  Memory::set_duck_written_by_dirty(app.start_addr,
                                    app.special_region_break_addr);
  ...
  uint64_t temp_pages = Memory::count_accessed_pages(
      app.start_addr, app.special_region_break_addr);
  temp_pages -=
      Memory::count_accessed_pages(app.stdin_start_addr, app.stdin_break_addr);
  ...
  if (app.duckinfo_ptr) {
    res.stdout_size =
        std::min(app.duckinfo_orig.stdout_limit, app.duckinfo_ptr->stdout_size);
  } else {
  ...
  if (app.duckinfo_ptr) {
    res.stderr_size =
        std::min(app.duckinfo_orig.stderr_limit, app.duckinfo_ptr->stderr_size);
  } else {
