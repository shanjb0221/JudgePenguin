...
static bool load_elf64(const char *buf, uint32_t len, const AppConfig &config, App &app) {
    LDEBUG_ENTER_RET();
    ...
    if (hdr->e_ident[EI_OSABI] != ELFOSABI_SYSV && hdr->e_ident[EI_OSABI] != ELFOSABI_LINUX) return false;
...
RunResult run(const App &app, uint64_t time_limit_ns) {
    ...
    if (app.duckinfo_ptr) {
        Trap::run_user_64(app.entry, app.rsp, time_limit_ns, res.time_tsc, res.trap_num, res.return_code);
    }
    ...
    if (app.duckinfo_ptr) {
        res.stdout_size = std::min(app.duckinfo_orig.stdout_limit, app.duckinfo_ptr->stdout_size);
    } else {
        res.stdout_size = std::min(app.duckinfo_orig.stdout_limit, (uint64_t)app.duckinfo32_ptr->stdout_size);
    }

    // stderr
    res.stderr_ptr = app.duckinfo_orig.stderr_ptr;
    if (app.duckinfo_ptr) {
        res.stderr_size = std::min(app.duckinfo_orig.stderr_limit, app.duckinfo_ptr->stderr_size);
    } else {
        res.stderr_size = std::min(app.duckinfo_orig.stderr_limit, (uint64_t)app.duckinfo32_ptr->stderr_size);
    }
