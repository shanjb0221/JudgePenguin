...
static inline uint64_t P4_index(uint64_t vaddr) { return (vaddr >> 39) & (PAGE_SIZE / 8 - 1); }

static inline uint64_t P3_index(uint64_t vaddr) { return (vaddr >> 30) & (PAGE_SIZE / 8 - 1); }

static inline uint64_t P2_index(uint64_t vaddr) { return (vaddr >> 21) & (PAGE_SIZE / 8 - 1); }

static inline uint64_t P1_index(uint64_t vaddr) { return (vaddr >> 12) & (PAGE_SIZE / 8 - 1); }

// Get recursively mapped first level PTE
static inline uint64_t &get_P1(uint64_t vaddr) { return *(uint64_t *)(((vaddr >> 9) & -8) | -(1ull << 39)); }

// Get recursively mapped second level PTE
static inline uint64_t &get_P2(uint64_t vaddr) { return *(uint64_t *)(((vaddr >> 18) & -8) | -(1ull << 30)); }

// Get recursively mapped third level PTE
static inline uint64_t &get_P3(uint64_t vaddr) { return *(uint64_t *)(((vaddr >> 27) & -8) | -(1ull << 21)); }

// Get recursively mapped fourth level PTE
static inline uint64_t &get_P4(uint64_t vaddr) { return *(uint64_t *)(((vaddr >> 36) & -8) | -(1ull << 12)); }
...
static void init_page_tables() {
    ...
    page_table_break = header.kernel_base;
    kernel_break = header.kernel_stack_top;
    vaddr_break = RESERVED_MEM_SIZE;
...
uint64_t get_kernel_break() { return kernel_break; }

uint64_t get_vaddr_break() { return vaddr_break; }
...
void set_page_flags_user_writable(uint64_t start, uint64_t end) {
    ...
    while (start != end) {
        ...
        P1 = clear_page_flags(P1) | PTE_PRESENT | PTE_USER | PTE_WRITABLE | PTE_NO_EXECUTE;
...
void map_region_cache_disabled(uint64_t start, uint64_t end, uint64_t src_addr) {
    ...
    while (start != end) {
        uint64_t &P1 = get_P1(start);
        P1 = src_addr | PTE_PRESENT | PTE_WRITABLE | PTE_CACHE_DISABLE | PTE_WRITE_THROUGH;
