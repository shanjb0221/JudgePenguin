...
/*
static bool process_controls(char *content, int len, char *&res, int
&res_len) {
  res = NULL;
  res_len = -1;
  content[len] = 0;
  ...
  if (equals_to(content, len, "uptime")) {
    ...
    using x86_64::rdmsr;
    uint64_t temp_target = extract_bits(rdmsr(MSR_TEMPERATURE_TARGET), 23,
    16); uint64_t temp =
        temp_target - extract_bits(rdmsr(IA32_THERM_STATUS), 22, 16);
...
static bool process_data(char *content, int len, char *&res, int &res_len) {
  ...
  } else if (3 == sscanf(content, "copy-buffer %lu %lu %lu", &q_off, &q_off2,
                         &q_len)) {
    if (Judger::copy_buffer(q_off, q_off2, q_len)) {
      res = content;
      sprintf(res, "ok-copy-buffer %lu %lu %lu", q_off, q_off2, q_len);
    }
  } else if (3 == sscanf(content, "compare-buffer %lu %lu %lu", &q_off,
  &q_off2,
                         &q_len)) {
    bool comp_result;
    if (Judger::compare_buffer(q_off, q_off2, q_len, comp_result)) {
      res = content;
      sprintf(res, "ok-compare-buffer %lu %lu %lu %lu", q_off, q_off2, q_len,
              (uint64_t)comp_result);
    }
  } else {
    return false;
  }
...
static bool process_judge(char *content, int len, char *&res, int &res_len) {
  ...
  if (16 ==
      sscanf(content,
             ...
             &req.ELF.off, &req.ELF.len, &req.stdin.off, &req.stdin.len,
             &req.stdout.off, &req.stdout.len, &req.stderr.off,
             &req.stderr.len, &req.IB.off, &req.IB.len, &req.OB.off,
             &req.OB.len, &req.OB_need_clear)) {
    ...
    if (j_res.error) {
      ...
      res_len = snprintf(
          ...
          "trap-cr2 0x%lx\n"
          "status %s\n",
          j_res.count_inst, j_res.clk_thread, j_res.clk_ref_tsc, clock_MHz,
          tsc_MHz, ext_MHz, j_res.time_ns, j_res.time_ns_ref_tsc,
          j_res.time_ns_real, j_res.time_tsc, j_res.memory_kb,
          j_res.memory_kb_accessed, j_res.stdout_size, j_res.stderr_size,
          j_res.return_code, (int32_t)(uint32_t)j_res.trap_num,
          j_res.trap_epc, j_res.trap_cr2, j_res.is_RE    ? RE_STR :
          j_res.is_TLE ? TLE_STR
                         : FINISH_STR);
    }
  } else {
    return false;
...
static bool process_cache(char *content, int len, char *&res, int &res_len) {
  ...
  if (len > 64 && 3 == sscanf(content, "load-cache %s %lu %lu", q_cache_name,
                              &q_off, &q_len)) {
    res = content;
    if (Judger::load_cache(q_cache_name, q_off, q_len, content + len - 64)) {
      res_len =
          sprintf(res, "ok-load-cache %s %lu %lu", q_cache_name, q_off,
          q_len);
    } else {
      res_len = sprintf(res, "fail-load-cache %s %lu %lu", q_cache_name,
      q_off,
                        q_len);
    }
  } else if (len > 64 && 3 == sscanf(content, "store-cache %s %lu %lu",
                                     q_cache_name, &q_off, &q_len)) {
    res = content;
    if (Judger::store_cache(q_cache_name, q_off, q_len, content + len - 64))
    {
      res_len =
          sprintf(res, "ok-store-cache %s %lu %lu", q_cache_name, q_off,
          q_len);
    } else {
      res_len = sprintf(res, "fail-store-cache %s %lu %lu", q_cache_name,
      q_off,
                        q_len);
    }
...
#define APPEND_RESULT() \
  { \
    memcpy(res + res_len, tmp_res, tmp_res_len); \
    res[(res_len += tmp_res_len)++] = '\n'; \
  }
...
TODO: here is the main entry of the judger
static int duck_packet_handle(DucknetIPv4Address src, uint16_t sport,
...
#define args content, content_len, to_send, to_send_len
    process_controls(args) || process_data(args) || process_judge(args) ||
        process_cache(args) || process_special(args);
#undef args
