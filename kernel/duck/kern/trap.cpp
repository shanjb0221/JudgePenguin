...
static uint64_t gdt[] = {0,
                         ...
                         [GDT_TSS >> 3] = (sizeof(TaskState) - 1) | (((uint64_t)&task_state & 0xffffff) << 16) |
                                          (9ul << 40) | (0ul << 45) | (1ul << 47) | (1ul << 53) |
                                          (((uint64_t)&task_state >> 24) << 56),
...
static struct {
    uint16_t lim;
    uint64_t addr;
} __attribute__((packed, aligned(16))) idt_desc = {.lim = sizeof(idt) - 1, .addr = (uint64_t)idt},
                                       gdt_desc = {.lim = sizeof(gdt) - 1, .addr = (uint64_t)gdt};
...
extern "C" {
uint64_t __syscall_stack;
}
static Trapframe tf_run_user, tf_from_user, tf_to_user;
static uint64_t user_time_limit_ns;

extern "C" void trap_return(Trapframe *tf);

extern "C" void syscall_return_record_tsc(Trapframe *tf);

extern "C" void syscall_return_32_record_tsc(Trapframe *tf);
...
static void set_idt(int id, void *addr) {
    ...
    idt[id] = (InterruptDescriptor){.ptr_low = (uint16_t)a,
...
extern "C" void trap_handler(Trapframe *tf) {
    // restore kernel tsc
    ...
    if (tf->tf_cs & 3) { // trap from user
        ...
        LDEBUG("trap %d, rip %lx, cr2 %lx, ec %lu", num, tf_from_user.tf_rip, cr2, tf_from_user.tf_errorcode);
        LDEBUG("tf_cs %lu, tf_rflags %lx, tf_rsp %lx, tf_ss %lu", tf_from_user.tf_cs, tf_from_user.tf_rflags,
        ...
        if (num == TRAP_IRQ + PIC::IRQ_TIMER) {
            ...
            uint64_t cr2;
            __asm__ volatile("movq %%cr2, %0" : "=r"(cr2));
            LERROR("trap %d, rip %lx, cr2 %lx, errorcode %lu", num, tf->tf_rip, cr2, tf->tf_errorcode);
            LERROR("tf_cs %lu, tf_rflags %lx, tf_rsp %lx, tf_ss %lu", tf->tf_cs, tf->tf_rflags, tf->tf_rsp, tf->tf_ss);
...
extern "C" void trap_0x80_handler(Trapframe *tf) {
...
void run_user_64(uint64_t entry, uint64_t rsp, uint64_t time_limit_ns, uint64_t &tsc, uint8_t &trap_num,
                 int32_t &return_code) {
    LDEBUG_ENTER_RET();
    ...
    LDEBUG("tsc1 = %lu tsc2 = %lu tscdiff = %lu", tf_from_user.tf_regs.tsc, tf_to_user.tf_regs.tsc,
           tf_run_user.tf_regs.tsc);
...
void run_user_32(uint64_t entry, uint64_t esp, uint64_t time_limit_ns, uint64_t &tsc, uint8_t &trap_num,
                 int32_t &return_code) {
    LDEBUG_ENTER_RET();
