...
//     LINFO("time %.6lf ms, memory %d KiB (%.1lf MiB) (A: %d KB)", res.time_ns / 1e6, res.memory_kb,
//           res.memory_kb / 1024.0, res.memory_kb_accessed);
//     LINFO("tsc %lu, trap %u, retcode %d", res.time_tsc, res.trap_num, res.return_code);
//     LINFO("stdout size %lu, stderr size %lu", res.stdout_size, res.stderr_size);
