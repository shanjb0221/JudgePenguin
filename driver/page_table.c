...
static inline u64 P2_index(virt_addr_t virt_addr) { return (virt_addr >> 21) & 0x1ff; }
static inline u64 P1_index(virt_addr_t virt_addr) { return (virt_addr >> 12) & 0x1ff; }
        ...
        *PTE(P3Low, P3Idx) = P2 | PTE_P | PTE_W | PTE_D | PTE_A | PTE_U;
        ...
        for (u64 P2Idx = 0; P2Idx < P1Num; ++P2Idx) {
            ...
            *PTE(P2, P2Idx) = P1 | PTE_P | PTE_W | PTE_D | PTE_A;
            u64 P0Num =
                min(div_round_up(total_size - P3Idx * P3_PAGE_SIZE - P2Idx * HUGE_PAGE_SIZE, PAGE_SIZE), PT_ENTRIES);
            for (u64 P1Idx = 0; P1Idx < P0Num; ++P1Idx) {
                u64 P0 = P3Idx * P3_PAGE_SIZE + P2Idx * HUGE_PAGE_SIZE + P1Idx * PAGE_SIZE;
